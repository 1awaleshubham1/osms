        ...
        while($row = $results->fetch_assoc())
        {
            echo '<div class="card mt-5 mx-5">';
                ...
                echo '<div class="card-body">';
                        ...
                        echo '<p class="card-text"> '.$row['requester_date'].' </p>';
                    echo '<div class="float-right">';
                        echo '<form action="" method="POST">';
                            echo '<input type="hidden" name="id" value='.$row["request_id"].'>';
                            echo '<input type="submit" class="btn btn-danger mr-3" value="View" name="View">';
                            echo '<input type="submit" class="btn btn-primary" value="Close" name="Close">';
                        echo '</form>';
                            echo '</div>';
                echo '</div>';
            echo '</div>';
...
if(isset($_REQUEST['assign']))
{
    ...
    else
    {
        ...
        $sql = "INSERT INTO assignwork_tb(request_id, request_info, requester_desc, requester_name, 
                ...
                VALUES('$rid', '$rinfo', '$rdesc', '$rname', '$radd1',
                '$radd2', '$rcity', '$rstate', '$rzip', '$remail', 
                ...
                else
                {
                    ...
                    $msg = '<div class="alert alert-danger col-sm-6 ml-5 mt-2"> Unable to Assign work </div>'.mysqli_error($conn);
                }
    }
}
?>
...
<div class="col-sm-5 mt-5 jumbotron">
    <form action="" method="POST">
        ...
        <div class="form-group">
            ...
            value="<?php if(isset($row['request_id'])) echo $_REQUEST['id']; ?>" name="request_id" readonly>
        </div>
        ...
        <div class="form-row">       
            ...
            <div class="form-group col-md-4">
                ...
                value="<?php if(isset($row['requester_zip'])) echo $row['requester_zip']; ?>" onkeypress="isInputNumber(event)">       
            </div>
        </div>    
        <div class="form-row">
            ...
            <input type="text" class="form-control" id="requesterzip" name="requestermobile" 
            value="<?php if(isset($row['requester_mobile'])) echo $row['requester_mobile']; ?>"onkeypress="isInputNumber(event)">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group col-md-6">
